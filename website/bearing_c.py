...
def bearing_c_iso(DL, LL, mxp, mxv, myp, myv, col, bc, fck, fyk, bar, covr):
    ...
    if DL >= 200 and DL <= 4100 and LL >= 130 and LL <= 2100 and col >= 0.1\
        ...
        def sig_prop(B, D, col, Df, DL, LL, mxp, mxv, myp, myv):
            ...
            return sig_p, ex, ey, SW_conc, SW_fill
        ...
        def B_D_rho(D, D_tmp, bc, DL, LL, mxp, mxv, myp, myv, B, fyk, rho, rho_min):
            sig_p, ex, ey, SW_conc, SW_fill = sig_prop(B, D, col, Df, DL, LL, mxp, mxv, myp, myv)
            if sig_p > bc:
                while sig_p > bc:
                    sig_p, ex, ey, SW_conc, SW_fill = sig_prop(B, D, col, Df, DL, LL, mxp, mxv, myp, myv)
                    B += 0.000005
            ...
            if med < mrd:
                ...
                while med >= mrd:
                    med, mrd, z = med_mrd(sig_s, B, col, D, phi, fck, rho, rho_min, fyk)
                    rho += 0.000000005
            return D, B, rho, p_s, ex, ey, sig_p, D_wide, D_punch, ved_wide,\
        ...
        D_final, B_final, rho_final, p_s, ex, ey, sig_p, D_wide, D_punch, ved_wide,\
            ved_punch, vrd, med, mrd, SW_conc, SW_fill, sig_s, d_wide, d_punch,\
                    vrd_wide, vrd_punch, k_wide, vrd_min_wide, Ap2_wide, As_wide,\
                    k_punch, vrd_min_punch, Ap2_punch, As_punch, z\
            = B_D_rho(D_initial, D_initial, bc, DL, LL, mxp, mxv, myp, myv, B_initial,\
        ...
        if D_final != D_initial or B_final != B_initial or rho_final != rho_initial:
            D_final, B_final, rho_final, p_s, ex, ey, sig_p, D_wide, D_punch, ved_wide,\
                ved_punch, vrd, med, mrd, SW_conc, SW_fill, sig_s, d_wide, d_punch,\
                    vrd_wide, vrd_punch, k_wide, vrd_min_wide, Ap2_wide, As_wide,\
                    k_punch, vrd_min_punch, Ap2_punch, As_punch, z\
                    = B_D_rho(D_final, D_final, bc, DL, LL, mxp, mxv, myp, myv, B_final,\
                        fyk, rho_final, rho_min)
